...
export async function POST(req: Request) {
  try {
    ...
    if (!user.email || user.email.toLowerCase() !== email.toLowerCase()) {
      console.log(`[Forgot Password] FAILED: Email mismatch. DB: "${user.email}", Input: "${email}"`);
      return NextResponse.json({ error: 'Email does not match owner email' }, { status: 400 });
    }

    console.log(`[Forgot Password] User validation SUCCESS! Proceeding to generate token...`);
